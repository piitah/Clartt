...
(function ($) {
	...
	$.fn.mDatatable = function (options) {
          ...
          datatable.jsonData = datatable.jsonData || [];
			...
			getData: function () {
				...
				if (options.data.type === 'remote') {
					...
					params.method = API.getOption('data.source.read.method'); // NOTE: THIS FIXES A BUG IN THE SOURCE CODE OF THE THEME THAT WOULDN'T ALLOW GET REQUESTS
				...
				return $.ajax(params)
					...
					.fail(function (jqXHR, textStatus, errorThrown) {
						...
						$('<span/>').addClass('m-datatable--error').css('visibility', 'visible').width('100%').html(API.getOption('translate.records.apiError')).appendTo(datatable.tableBody); // NOTE custom error instead of nothing
						$(datatable).addClass('m-datatable--error');
					})
					.always(function () {
					});
			...
			localDataUpdate: function () {
				var params = API.getDataSourceParam();
				//if (typeof datatable.fullJsonData === 'undefined') { // NOTE ANOTHER BUG INSIDE OF THE THEME. ON REFRESH datatable.fullJsonData is not undefined and therefore is not assigned after refresh
        ...
        console.log('inside of search 2');
				var delay = (function () {
          console.log('delay');
					return function (callback, ms) {
						clearTimeout(API.timer);
